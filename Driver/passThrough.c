...
#define USERMODE_NAME "usermode_ex.ex"
...
{
    //https://www.vergiliusproject.com/kernels/x64/Windows%2011/22H2%20(2022%20Update)/_EPROCESS
...

{
    ...
    if (Data->Iopb->MajorFunction == IRP_MJ_WRITE) {
        ...
        if (Data->Iopb->Parameters.Write.WriteBuffer && Data->Iopb->Parameters.Write.Length) {
            PEPROCESS client_proc = 0;
            FindProcessByName(USERMODE_NAME, &client_proc);


            auto proc = FltGetRequestorProcess(Data);
            if (client_proc && proc && (DWORD32)client_proc == proc)
            {
                struct AES_ctx ctx;
                AES_init_ctx_iv(&ctx, KEY, IV);
                AES_CTR_xcrypt_buffer(&ctx, Data->Iopb->Parameters.Write.WriteBuffer, Data->Iopb->Parameters.Write.Length);
            }
...
)
{
    ...
    if (Data->Iopb->MajorFunction == IRP_MJ_READ) {
      ...
      if (Data->Iopb->Parameters.Read.ReadBuffer && Data->Iopb->Parameters.Read.Length) {
            ...
            if (client_proc && proc && (DWORD32)client_proc == proc)
            {
              struct AES_ctx ctx;
              AES_init_ctx_iv(&ctx, KEY, IV);
              AES_CTR_xcrypt_buffer(&ctx, Data->Iopb->Parameters.Read.ReadBuffer, Data->Iopb->Parameters.Read.Length);
            }
