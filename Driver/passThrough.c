...

{
    ...
    status = FltGetFileNameInformation(
        ...
        &NameInfo);
    UNICODE_STRING required_extension = RTL_CONSTANT_STRING(LAB_IN);
    if (NT_SUCCESS(status))
    {
        if (RtlEqualUnicodeString(&required_extension, &NameInfo->Extension, FALSE) == TRUE) {
            ...
            if (Data->Iopb->MajorFunction == IRP_MJ_WRITE) {
                ...
                if (Data->Iopb->Parameters.Write.WriteBuffer && Data->Iopb->Parameters.Write.Length) {
                   ...
                   //
                   //xorvalue = value ^ xorkey
                   //valaue == xorvalue ^ xorkey
                   //
